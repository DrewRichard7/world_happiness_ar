...
*to knit to the correct directory, run the following in the console*: `rmarkdown::render(here("notebooks/calculations.Rmd"), output_dir = here("assets"))`

**First we'll import the libraries we're going to use.**

```{r}
shh <- suppressMessages
shh(here::i_am("notebooks/calculations.Rmd"))
shh(library(here))
...
```{r}
data <- tibble(read_csv(here('data/happy_sad_sugar_fish.csv'), show_col_types = FALSE)) |> 
  arrange(country) |> 
